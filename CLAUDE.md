...
- `my.<x>` options only when parametrised (`my.firewall`, `my.docker`, `my.autoUpdate`, `my.gc`, `my.secrets`, `my.repo`, `my.installer`, HM `my.dotfiles`). `src/modules/secrets.nix` declares all sops secrets and is inert until `secrets/common.yaml` exists.
- `src/modules/dotfiles.nix` (HM aspect, in `profiles.base`): links every top-level dir of the dotfiles checkout (`my.dotfiles.path`, default `~/personal/dotfiles`) into `~/.config`. No defaults are shipped in this repo — missing checkout/entry means the program's own defaults. Never require the checkout.
