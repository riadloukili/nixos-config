...
# candidates older than a threshold, then collects garbage and optimises.
# (nix.gc.automatic can only do one of the two.)
{
  flake.modules.nixos.core-gc =
    {
      config,
      lib,
      pkgs,
    ...
    in
    {
      options.my.gc = {
        enable = lib.mkOption {
          type = lib.types.bool;
          default = true;
          description = "Run the generation-aware garbage collector on a timer.";
        };
        ...
        time = lib.mkOption {
          type = lib.types.str;
          default = "03:00";
          description = "Daily run time (HH:MM).";
        };
      };
      ...
      config = lib.mkIf cfg.enable {
        ...
        systemd.services.nix-gc-generations = {
          ...
          script = ''
            ...
            echo "generations: $total, keeping newest $keep, deleting those older than ${cfg.deleteOlderThan}"
